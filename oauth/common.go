...
func (c *commonController) syncTokenData(ctx context.Context, exchange *exchangeResult) error {
	...
	k8sClient, err := c.ClientFactory.CreateClient(context.WithValue(ctx, "namespace", exchange.TokenNamespace))
...
func (c *commonController) checkIdentityHasAccess(ctx context.Context, state *oauthstate.OAuthInfo) (bool, error) {
	...
	k8sClient, err := c.ClientFactory.CreateClient(context.WithValue(ctx, "namespace", state.TokenNamespace))
	if err != nil {
